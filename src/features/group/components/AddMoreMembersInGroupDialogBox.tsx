...
import { addNewMembersInGroup } from "../api/api";
...
const AddMoreMembersInGroupDialogBox = ({ userId, selectedGroup }:AddMoreMembersPropsType) => {
  ...
  const filterData = data?.filter((user: UserType) => {
...
const addMembers = (id: string) => {
  ...
  const AddNewMembersInGroup = async () => {
    try {
      await addNewMembersInGroup({groupId:selectedGroup?.id , addedMembers})
                ...
                return (
                  <div className="bg-white px-2 py-3 rounded-sm flex justify-between items-center">
                    <div className="flex gap-2">
                      <img
                        src={u.profileUrl || "https://github.com/shadcn.png"}
                        alt=""
